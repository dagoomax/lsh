              ...
              return (
                <div key={s.path} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:4 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:4, overflow:'hidden', flex:1 }}>
                    <Icon size={12} color={on ? 'var(--tile-on-ink)' : 'var(--text3)'} />
                  ...
                  {isToggle
                    ...
                    : isReadBool
                      ? <span style={{ fontSize:10, color: on ? 'var(--tile-on-ink)' : 'var(--text3)', fontWeight:600 }}>{on ? 'Yes' : 'No'}</span>
          ...
          <div style={{ marginTop:6, display:'flex', flexDirection:'column', gap:3 }}>
            ...
            {smartthingsSensors.slice(0,5).map(s => {
              ...
              return (
                <div key={s.path} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:4 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:4, overflow:'hidden', flex:1 }}>
                    <Icon size={12} color={on || (isRange && s.value > 0) ? 'var(--tile-on-ink)' : 'var(--text3)'} />
          ...
          <div style={{ marginTop:6, display:'flex', flexDirection:'column', gap:3 }}>
            ...
            {tradfriSensors.slice(0,5).map(s => {
              ...
              return (
                <div key={s.path} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', gap:4 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:4, overflow:'hidden', flex:1 }}>
                    <span style={{ fontSize:12, color:'var(--tile-on-ink)' }}>◆</span>
                    <span style={{ fontSize:10, color:'var(--text2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{s.name}</span>
                  </div>
                  {isToggle
...
export default function DeviceList({ devices, energy, roomsMeta = {}, onToggleRelay }) {
  ...
  // EV chargers (up to 10, one card each on the Energy tab — see
  // EnergyFlow.jsx) rather than routed through the solar/battery/grid/loads
  // brand picker above, which exists to mix two specific brands per metric,
  // not to total an arbitrary number of charger devices.
  const evDevices = useMemo(() => devices.filter(d => d.category === 'ev-charger').slice(0, 10), [devices])
  ...
  return (
    <div style={{ display:'flex', height:'100%', overflow:'hidden' }}>
      ...
      <div className="device-sidebar" style={{
        ...
        {/* Rooms (from device customizations) */}
        {rooms.length > 0 && (
