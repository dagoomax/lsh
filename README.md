...
- `model` — which COCO-SSD base model to run **by default**: `lite_mobilenet_v2` (default, fastest), `mobilenet_v2` (more accurate, slower), or `mobilenet_v1` (smallest download). Changeable live from **Settings → 📷 Cameras → AI Camera Detection** — picking a model downloads it fresh from its CDN and validates it loads before saving, so a bad/offline pick doesn't silently break detection on next restart. Weights aren't vendored, so every option is fetched over the network the first time it's selected (`GET`/`POST /api/settings/object-detection/model`)
- `cameras[].model` — optional per-camera override of the base model above, same three ids — a camera that mostly needs to know "was anyone there" can run the fastest model while one you actually want accurate breed/vehicle guesses from runs `mobilenet_v2`. Leave unset to use the default. Editable per-camera from the same Settings card; all distinct models in use (default + every override) are preloaded at startup so no poll ever blocks on a cold download
...
- `classWeights` — e.g. `{ "person": 10, "cat": 5, "kite": 0 }`. Not a physical weight — how much a class counts as *significant*. Every class still gets its own device/sensor/event-log entries no matter what; weight only gates two things: whether it gets an auto-created Flow, and whether it counts toward the per-camera aggregate `objectdetect/<slug>/any/detected` signal (used by [HKSV](#src-homekit-camerajs)'s motion trigger). Defaults to `1` (counts) for any class not listed; set a class to `0` to keep detecting it without it spamming a Flow or triggering a recording every time — e.g. a "kite" or "frisbee" blowing across frame
- `petVerification` — set `false` to skip loading the second model described below entirely (saves the extra memory/load time if you don't care about breed info or don't have pets)
- `requirePetVerification` — set `true` to *drop* a cat/dog/bird/horse detection outright when the second model's crop classification doesn't back it up (see below). Default `false`: still detect and log it, just tagged "unverified breed match"

**Pet breed verification (second model)** — COCO-SSD's cat/dog/bird/horse classes are coarse (just "this is a cat"), so when one of those four classes is detected, the cropped region gets a second pass through [MobileNet](https://github.com/tensorflow/tfjs-models/tree/master/mobilenet) (ImageNet-1000, which has ~120 dog breeds plus several cat/bird classes) for a more specific guess — e.g. an event log entry reading `cat (82%) — Persian cat (74%)` instead of just `cat (82%)`. If none of MobileNet's top-3 guesses land anywhere near the expected animal type, the entry is tagged "unverified breed match" instead — a hint the original COCO-SSD call might have been a false positive, not proof either way (MobileNet can miss on a bad crop/angle same as COCO-SSD can). This only runs on the cat/dog/bird/horse classes, never the other 76 COCO-SSD classes. **Known gap**: ImageNet has essentially no generic "horse" class (its one horse-adjacent class, "sorrel", is actually a coat-color label) — horse verification will rarely find a match, which is an ImageNet limitation, not a bug here. Adds `@tensorflow-models/mobilenet` to the memory floor described below; set `petVerification: false` to skip loading it if you don't need this.
