...
- **`lsh/run.sh`** — bridges Supervisor's single persistent `/data` directory to the paths LSH's own code expects (`/app/config.json`, `/app/persist`, `/app/certs`), via symlinks, seeding an empty `config.json` on first run (not `config.example.json` — that file's sections are fully filled in with placeholder hosts and would auto-start every integration against a fake address; real config comes from LSH's own Settings UI).
