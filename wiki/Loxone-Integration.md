...
- `?type=<integration,…>` — filter by integration key (`fibaro`, `somfy`, `satel`, `unifi`, `shelly`, `vitodens`, …). `vicare` and `vitodens` are treated as the same group either way, since they're two separate clients for the same Viessmann brand.
