...
- `?type=<integration,…>` — filter by integration key (`fibaro`, `somfy`, `satel`, `unifi`, `shelly`, `vitodens`, …). `?type=vicare` still works as an alias for `vitodens` (its older, now-merged name).
